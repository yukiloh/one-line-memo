...
2. (重要)sleep没有释放锁,wait释放了
3. sleep可以在任何地方使用.wait(还有notify等)需要在同步代码中(例如synchronized)执行
4. sleep需要捕获异常.wait如果锁的是自己,则不需要捕获异常(反之,如果ide提示需要抛出异常则代表锁的不是自己)
...
被volatile变量修饰的共享变量进行写操作的时候会多出一行*lock*前缀的汇编代码  
并且,jvm以此创建内存屏障  
对指令排序时,保证在这之前的代码不会在lock后执行,在这之后的代码不会在lock之前执行  
...
#### `==`比较的是内存地址,object的`equals`其实也是.但String的equals进行了"重写"
